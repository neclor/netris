...
var _game_over: bool
var swap_allowed: bool:
	set = set_swap_allowed
...
func step() -> void:
	if paused or _game_over: return
	try_move_figure(Figure.Direction.DOWN)
...
func set_swap_allowed(value: bool) -> void:
	if swap_allowed == value: return
	swap_allowed = value
	swap_allowed_changed.emit(value)


func try_move_figure(direction: Figure.Direction) -> bool:
	if paused or _game_over: return false
	if _try_move_figure(current_figure, direction):
		current_figure_changed.emit(current_figure)
		return true
...
func try_swap_figure() -> bool:
	if paused or _game_over or not swap_allowed: return false
	if (hold_figure == null):
		hold_figure = current_figure
		swap_allowed = false
		_spawn_next_figure()
		hold_figure_changed.emit(hold_figure)
		return true
...
func _try_move_figure(figure: Figure, direction: Figure.Direction = Figure.Direction.DOWN) -> bool:
	...
	if field.check_figure_collides(new_figure):
		return false
...
func _place_current_figure() -> void:
	...
	_game_over = field.check_game_over()
	if _game_over: 
		game_over.emit()
		return
	_spawn_next_figure()
...
func _check_bonus_figure() -> void:
	if not field.check_figure_collides(bonus_figure): return

	var new_score: int = 50
	for block in bonus_figure.get_block_positions():
		if field.get_block(block) != Field.EMPTY_VALUE and (field.get_block(block) as Figure.Type) == bonus_figure.type:
			new_score = 100

	if bonus_figure.is_equal(current_figure):
		new_score = 500
...
func _spawn_bonus_figure() -> void:
	...
	_drop_figure(figure)
	bonus_figure = figure
	bonus_figure_changed.emit(bonus_figure)
...
func _update_speed() -> void:
	speed = 2 + destroyed_line_count / 8 * 0.5
